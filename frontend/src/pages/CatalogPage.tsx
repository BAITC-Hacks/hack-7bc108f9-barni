...
export default function CatalogPage({ role }: { role: CurrentDemoRole }) {
  ...
  const [readinessOptions, setReadinessOptions] = useState<Array<{ slug: ReadinessLevel; label: string }>>([]);
  ...
  useEffect(() => {
    ...
    api.getMeta().then((meta) => {
      ...
      setReadinessOptions(meta.readiness_levels);
    }).catch((caught) => {
  ...
  function readinessLabel(value: ReadinessLevel): string {
    return readinessOptions.find((item) => item.slug === value)?.label ?? value;
  }
  ...
  return <div className="page-width catalog-page">
    ...
    {metaError && <div className="error-banner" role="alert">
      <span>Не удалось загрузить справочник фильтров: {metaError}</span>
      <button type="button" onClick={() => setReload((value) => value + 1)}>Повторить</button>
    </div>}
      ...
      <div className="filter-group">
        ...
        <label>Готовность
          <select value={readiness} onChange={(event) => setReadiness(event.target.value as ReadinessLevel | '')}>
            ...
            {readinessOptions.map((item) => <option value={item.slug} key={item.slug}>{item.label}</option>)}
          </select>
        </label>
        <label>Сортировка
          <select value="score_desc" disabled>
        ...
        return <article className="task-row" key={task.id}>
          ...
          <div>
            ...
            <div className="task-row__foot">
              {task.missing_fields.length} недостающих полей · Предложений: {task.proposals_count} · Опубликовано {formatDate(task.published_at)}
            </div>
          </div>
