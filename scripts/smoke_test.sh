...
REF_CARD='{"card":{"title":"Эталон","topic":"analytics","context":"Отчёты собираются вручную","need":"Автоматизировать сбор отчётов","data":"Выгрузки из CRM","expected_result":"Готовый дашборд продаж","success_criteria":"Отчёт за пять минут"}}'
...
call GET "/api/proposals?team_id=$ALPHA" 200 | jq -e --arg id "$P_ALPHA" 'any(.[]; .id == $id and .status == "accepted" and .task_title == "Оптимизация обработки заявок")' >/dev/null
